          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {propertyImages.map((card, index) => (
              <div 
                ...
                onClick={() => openPopup(card, hoveredCard === card.id ? hoveredImageIndex : 0)}
              >
                <div className="aspect-[4/3] overflow-hidden relative">
                  <img 
                    src={card.images[hoveredCard === card.id ? hoveredImageIndex : 0].url} 
                    alt={card.images[hoveredCard === card.id ? hoveredImageIndex : 0].alt}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                    loading="lazy"
                  />
                  
                  {/* Navigation arrows for hover slideshow */}
                      ...
                      {card.images.map((_, idx) => (
                        <div
                          ...
                          className={`w-2 h-2 rounded-full transition-all duration-300 ${
                            idx === (hoveredCard === card.id ? hoveredImageIndex : 0)
                              ? 'bg-white scale-125'
                              : 'bg-white/50'
                          }`}
                        />
